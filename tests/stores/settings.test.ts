...
describe('settings store', () => {
  ...
  it('sanitizes forecast options', () => {
    const s = useSettingsStore()
    s.load()
    expect(s.settings.forecast).toEqual({ enabled: true, lookbackMinutes: 60, minPoints: 3, notify: true })
    s.update({ forecast: { enabled: false, lookbackMinutes: 120, minPoints: 5, notify: false } })
    expect(s.settings.forecast).toEqual({ enabled: false, lookbackMinutes: 120, minPoints: 5, notify: false })
    s.update({ forecast: { enabled: true, lookbackMinutes: 45 as never, minPoints: 99, notify: 'yes' as never } })
    expect(s.settings.forecast).toEqual({ enabled: true, lookbackMinutes: 120, minPoints: 10, notify: false })
