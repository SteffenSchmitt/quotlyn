...
describe('timelineBars', () => {
  ...
  it('creates one row per window with a reset, in account then window order', () => {
    ...
    expect(bars).toEqual([
      { row: 0, accountName: 'Alpha', windowKey: '5h', startMs: now, endMs: Date.parse('2026-09-21T14:00:00.000Z'), utilization: 0.5 },
      { row: 1, accountName: 'Beta', windowKey: '7d', startMs: now, endMs: Date.parse('2026-09-23T12:00:00.000Z'), utilization: 0.9 },
    ])
  })
