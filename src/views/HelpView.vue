...
<template>
  <section class="max-w-2xl space-y-8">
    <div>
      <h2 class="text-lg font-bold">{{ t('help_page.title') }}</h2>
      <p class="mt-2 text-sm leading-relaxed text-slate-600 dark:text-slate-300">{{ t('help_page.intro') }}</p>
    </div>

    <div>
      <h3 class="text-sm font-bold text-slate-500">{{ t('help_page.steps.title') }}</h3>
      <ol class="mt-3 space-y-5">
        <li v-for="(s, i) in steps()" :key="i" class="flex gap-4">
          <span
            class="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-slate-800 text-xs font-bold text-white dark:bg-slate-200 dark:text-slate-900"
          >
            {{ i + 1 }}
          </span>
          <div class="min-w-0 flex-1">
            <p class="text-sm font-bold">{{ s.h }}</p>
    ...
    <div>
      <h3 class="text-sm font-bold text-slate-500">{{ t('help_page.notes.title') }}</h3>
